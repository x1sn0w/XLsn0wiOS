...
#pragma mark 保存用户信息
+ (void)insertUserModel:(XLsn0wArchiverModel *)model {
...
#pragma mark 取出用户信息
+ (XLsn0wArchiverModel *)selectFromUserModel {
    XLsn0wArchiverModel *model = [ArchiveTool unarchiveFromPath:[ArchiveTool pathWithKey:KUserArchivePathKey] withKey:KEY];
    if (!model) {
        model = [[XLsn0wArchiverModel alloc] init];
    }
    return model;
}

+ (void)deleteUserModel {
    XLsn0wArchiverModel *model = [[XLsn0wArchiverModel alloc] init];

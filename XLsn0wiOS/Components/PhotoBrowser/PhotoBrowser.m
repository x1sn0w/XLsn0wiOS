...
#define kPadding 20
#define PhotoBrowserSystemVersion [[UIDevice currentDevice].systemVersion doubleValue]
...
@interface PhotoBrowserItem()<NSCopying>
...
- (id)copyWithZone:(NSZone *)zone {
    PhotoBrowserItem *item = [self.class new];
    return item;
}
@end
...
@interface PhotoBrowserScrollView : UIScrollView <UIScrollViewDelegate>
...
- (instancetype)initWithGroupItems:(NSArray *)groupItems {
    ...
    if (PhotoBrowserSystemVersion >= 7) {
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        ...
        if([fromView isEqual:((PhotoBrowserItem *)self.groupItems[i]).thumbView]){
            page = (int)i;
            break;
        }
    }
...
//    [[UIApplication sharedApplication] setStatusBarHidden:YES withAnimation:animated ? UIStatusBarAnimationFade : UIStatusBarAnimationNone];
    ...
    PhotoBrowserScrollView *cell = [self cellForPage:self.currentPage];
    PhotoBrowserItem *item = _groupItems[self.currentPage];
...
- (BOOL)prefersStatusBarHidden {
    if (_fromNavigationBarHidden) {
        return _fromNavigationBarHidden;
    } else {
        return YES;
    }
}
...
//    [[UIApplication sharedApplication] setStatusBarHidden:_fromNavigationBarHidden withAnimation:animated ? UIStatusBarAnimationFade : UIStatusBarAnimationNone];
    ...
    PhotoBrowserScrollView *cell = [self cellForPage:currentPage];
    PhotoBrowserItem *item = _groupItems[currentPage];
...
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(PhotoBrowserScrollView *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            PhotoBrowserScrollView *cell = [self cellForPage:i];
            if (!cell) {
                PhotoBrowserScrollView *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (PhotoBrowserScrollView *cell in _cells) {
        if (cell.superview) {
...
- (PhotoBrowserScrollView *)dequeueReusableCell {
    PhotoBrowserScrollView *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
    ...
    cell = [PhotoBrowserScrollView new];
...
- (PhotoBrowserScrollView *)cellForPage:(NSInteger)page {
    for (PhotoBrowserScrollView *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    CGSize size = [msg sizeWithFont:font constrainedToSize:CGSizeMake(200, 200) lineBreakMode:NSLineBreakByCharWrapping];
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    PhotoBrowserScrollView *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {

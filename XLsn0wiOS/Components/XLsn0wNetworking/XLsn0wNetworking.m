...
-  (AFSecurityPolicy *)httpsSecurityPolicy {
    //先导入证书，找到证书的路径
    NSString *cerPath = [[NSBundle mainBundle] pathForResource:@"xiaolvlan" ofType:@"cer"];
...
+ (AFHTTPSessionManager *)shared {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[XLsn0wNetworkingManager alloc]initWithBaseURL:[NSURL URLWithString:ServerDomain]];
...
+ (void)uploadPostImage:(UIImage *)image purposeId:(NSString *)purposeId handler:(void (^)(id))handler{
    [self uploadImage:image purpose:@"forumBbs" purposeId:purposeId handler:handler];
}

+ (void)uploadImage:(UIImage *)image purpose:(NSString *)purpose purposeId:(NSString *)purposeId handler:(void(^)(id))handler{
    ...
    NSURLSessionDataTask *uploadTask = [[self shared] POST:@"file/uploadFile" parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
- (void)deleteImage:(NSString *)urlString parameters:(id)parameters success:(void (^)(NSURLSessionDataTask *task, id responseObject))success failure:(void (^)(NSURLSessionDataTask *task, NSError *error))failure{
    [self DELETE:urlString parameters:parameters success:success failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
...
+ (void)uploadImageWithPOST:(NSString*_Nullable)URLString
    ...
    XLsn0wNetworkingManager *manager = [self shared];

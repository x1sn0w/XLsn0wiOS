...
#import "PushBanner+Categories.h"
...
@implementation PushBanner

static NSMutableArray <PushBanner*>*sharedBannerViews;
...
+(instancetype)bannerWithBlock:(void(^)(EBBannerViewMaker *make))block{
    ...
    PushBanner *bannerView = [PushBanner bannerViewWithStyle:maker.style];
...
+(instancetype)current{
    PushBanner *view = sharedWindow.rootViewController.view.subviews.lastObject;
    if ([view isKindOfClass:[PushBanner class]] && view.superview) {
        return view;
    } else {
        return nil;
    }
...
-(void)show{
    ...
    [UIView animateWithDuration:_maker.showAnimationDuration delay:0 usingSpringWithDamping:damping initialSpringVelocity:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        PushBanner *strongSelf = weakSelf;
...
+(void)showWithContent:(NSString*)content{
    [[PushBanner bannerWithBlock:^(EBBannerViewMaker *make) {
...
+ (instancetype)bannerViewWithStyle:(EBBannerViewStyle)style{
    PushBanner *bannerView;
    for (PushBanner *view in sharedBannerViews) {

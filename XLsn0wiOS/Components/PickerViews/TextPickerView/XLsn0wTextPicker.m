...
@interface XLsn0wTextPicker ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
@implementation XLsn0wTextPicker
...
+ (void)showStringPickerWithTitle:(NSString *)title
    ...
    XLsn0wTextPicker *strPickerView = [[XLsn0wTextPicker alloc]initWithTitle:title dataSource:dataSource defaultSelValue:defaultSelValue isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (void)configDataSource:(id)dataSource defaultSelValue:(id)defaultSelValue {
    ...
    if (self.type == BRStringPickerComponentSingle) {
        if (defaultSelValue && [defaultSelValue isKindOfClass:[NSString class]] && [defaultSelValue length] > 0 && [self.dataSourceArr containsObject:defaultSelValue]) {
            ...
            self.row = [self.dataSourceArr indexOfObject:self.selectValue];
        } else {
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    textLabel.font = [UIFont systemFontOfSize:18.0f * kScaleFit];
    // 字体自适应属性
    textLabel.adjustsFontSizeToFitWidth = YES;
    // 自适应最小字体缩放比例
    textLabel.minimumScaleFactor = 0.5f;
    if (self.type == BRStringPickerComponentSingle) {
        textLabel.frame = CGRectMake(0, 0, self.alertView.frame.size.width, 35.0f * kScaleFit);
        textLabel.text = self.dataSourceArr[row];
    } else if (self.type == BRStringPickerComponentMore) {
        textLabel.frame = CGRectMake(0, 0, self.alertView.frame.size.width / [self.dataSourceArr[component] count], 35.0f * kScaleFit);
        textLabel.text = self.dataSourceArr[component][row];
    }
...
- (void)clickRightBtn {
    ...
    if(_resultBlock) {
        if (self.type == BRStringPickerComponentSingle) {
            _resultBlock(self.selectValue, self.row);
        } else if (self.type == BRStringPickerComponentMore) {
            _resultBlock(self.selectValueArr, self.row);
        }
    }
}

...
#define GetReuseIdentifier(cell) [NSString stringWithFormat:@"%@_%ld_%ld",NSStringFromClass([cell class]), [indexPath section], [indexPath row]];

#define GetCellForRowAtIndexPath(tableView, indexPath) [tableView cellForRowAtIndexPath:indexPath]
...
#define GetPopToViewController(vc) [self popToViewController:[vc class]]
...
//是否是空对象
#define isObjNull(obj) (obj == nil \
||  [obj isKindOfClass:[NSNull class]] \
|| ([obj respondsToSelector:@selector(length)] && [(NSData *)obj length] == 0) \
|| ([obj respondsToSelector:@selector(length)] && [(NSString *)obj length] == 0) \
|| ([obj respondsToSelector:@selector(count)]  && [(NSDictionary *)obj count] == 0) \
|| ([obj respondsToSelector:@selector(count)]  && [(NSArray *)obj count] == 0))

///对象不为空
#define isNotNull(obj)   (!isObjNull(obj))
#define isNull(obj)      (obj == nil \
||  [obj isKindOfClass:[NSNull class]] \
|| ([obj respondsToSelector:@selector(length)] && [(NSData *)obj length] == 0) \
|| ([obj respondsToSelector:@selector(length)] && [(NSString *)obj length] == 0) \
|| ([obj respondsToSelector:@selector(count)]  && [(NSDictionary *)obj count] == 0) \
...
#define SafeArea_Bottom_Height (iPhoneX ? 34 : 0)
...
#define initCellWithIdentifier(tableView, reuseIdentifier) [tableView dequeueReusableCellWithIdentifier:reuseIdentifier]

#define initCellWithStyle(cell, reuseIdentifier) [[cell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:reuseIdentifier]
...
#define Series_X (iPhoneXs_Max ||iPhoneX || iPhoneXr)

#define PriceFormat(price) [NSString convertPriceFormat:price]
#define PriceDoubleFormat(priceDouble) [NSString priceFormatFromDouble:priceDouble]
/*
...
#define MaskBackgroundColor      RGBACOLOR(0, 0, 0, 0.4)

#define PriceRMBFormat(price)    [NSString stringWithFormat:@"¥%.2f", price]
#define PriceStringFormat(price) [NSString stringWithFormat:@"%.2f", price]
...
/// Debug调试模式下控制台输出日志，Release生产环境下不再输出。(***发布产品时必须不能打印日志***)
#ifndef __OPTIMIZE__
...
#define ButtonHighlighted(button, backgroundColor) [button setBackgroundImage:[UIImage imageWithColor:backgroundColor] forState:UIControlStateHighlighted];
//比例适配
#define WBaseWidth 375
#define WBaseHeight 667

#define RCIMTotalUnreadCount [[RCIMClient sharedRCIMClient] getTotalUnreadCount]

#define logJSON(obj) XLsn0wLog(@"JSON = %@", convertJSON(obj));
#define logObj(obj) XLsn0wLog(@"obj = %@", obj);
#define logCGFloat(float) XLsn0wLog(@"CGFloat = %f", float);
#define logNSInteger(int) XLsn0wLog(@"NSInteger = %ld", int);
...
/// 融云 App Key
#define RCIMAppKey @"k51hidwqkn8kb" ///测试100

#pragma mark 导航条的高度

#define NumberFromString(string) [NSNumber numberWithString:string]

...
static char typeCodeKey;
- (void)setTypeCode:(NSString *)typeCode {
    objc_setAssociatedObject(self, &typeCodeKey, typeCode, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSString *)typeCode {
    return objc_getAssociatedObject(self, &typeCodeKey);
}
...
- (NSInteger)mode {
    return [objc_getAssociatedObject(self, &modeKey) integerValue];
}
...
- (NSString *)getProductTypeFromTypeCode:(NSString *)typeCode {
    ...
    if (isGuiXiYaoCai) {
        ...
        productType = @"中药粉剂";
    } else if (isWaiYongGaoFang) {
        productType = @"外用膏药";
    } else if (isZhongChengYao) {
        productType = @"中成药";
    } else if (isMingFang) {
        productType = @"专家名方";
    } else if (isYangSheng) {

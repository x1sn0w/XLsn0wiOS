...
static char canResignKey;
- (BOOL)canResign {
    return objc_getAssociatedObject(self, &canResignKey);
}

- (void)setCanResign:(BOOL)canResign {
    objc_setAssociatedObject(self, &canResignKey, @(canResign), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
